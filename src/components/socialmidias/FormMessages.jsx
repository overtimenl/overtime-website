...
function Formanuncio(props) {
	...
	const { mensage, opcao, setMensage, setOpenModel, primery, secudary } = props;
	...
	const submitForm = (e)=>{
        e.preventDefault();
        //console.log(anuncio)
        if(mensage.work != '' && mensage.name != '' && mensage.conteudo != '' && mensage.data != '' && mensage.more != ''){
        	
        	if (opcao === "EDITAR") {
        		
        		axios.put('http://localhost:8000/messagedb/update', {
					    id: mensage.id, 
					    work: mensage.work, 
					    name: mensage.name,    
        		...
        		axios.post('http://localhost:8000/messagedb/create', {
				    id: '------',
				    work: mensage.work, 
				    name: mensage.name,    
				    conteudo: mensage.conteudo, 
